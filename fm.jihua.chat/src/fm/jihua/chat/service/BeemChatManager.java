...
public class BeemChatManager extends IChatManager.Stub {
	...
	private ChatAdapter getChat(Chat chat) {
		...
		boolean history = PreferenceManager.getDefaultSharedPreferences(
				...
				false);
		App app = (App)mService.getApplication();
		String accountUser = String.valueOf(app.getMyUserId())+"@"+App.CHAT_HOST;
		String historyPath = PreferenceManager.getDefaultSharedPreferences(
				mService.getBaseContext()).getString(App.CHAT_HISTORY_KEY, "");
		if ("".equals(historyPath))
			historyPath = "/Android/data/fm.jihua.chat/chat/";
		res.setHistory(history);
		res.setAccountUser(accountUser);
		res.setHistoryPath(new File(Environment.getExternalStorageDirectory(),
				historyPath));
		res.setDbHelper(mService.getDBHelper());
...
//					.getDefaultSharedPreferences(mService);
			try {
				CharSequence title = "来自课程格子的用户";
				// mService.getBind().getRoster()
				// .getContact(chat.getParticipant().getJID()).getName();
				
				App app = (App) mService.getApplicationContext();
				User user = app.getUser(chat.getParticipant().getJID());
				if (user != null) {
					title = user.name;
				}
				Notification notification = new Notification(
						app.getIcon(), title + ":" + msgBody,
						System.currentTimeMillis());
				
				notification.flags = Notification.FLAG_AUTO_CANCEL
						| Notification.FLAG_SHOW_LIGHTS;
				notification.setLatestEventInfo(mService, title, msgBody,
						makeChatIntent(chat));
