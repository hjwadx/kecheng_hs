...
//		if (mDbHelper != null) {
//			int offset = mMessages.size();
//			List<Message> messages = mDbHelper.getMessages(mDbHelper.getWritableDatabase(), mAccountUser, mAdaptee.getParticipant(), true, count, offset);
...
//	}
	 ...
	 */
	@Override
	public void sendMessage(fm.jihua.chat.service.Message message)
			throws RemoteException {
		fm.jihua.chat.service.Message encrypted = otrEncryptMessage(message);
		if (encrypted != null) {
	 ...
	 *            the message to send
	 */
	private void transferMessage(fm.jihua.chat.service.Message message) {
	...
	private void addMessage(Message msg) {
		...
		if (msg.getType() == Message.MSG_TYPE_CHAT) {
			mMessages.add(msg);
			mDbHelper.addMessage(mDbHelper.getWritableDatabase(), msg);
		}
	...
	public DatabaseHelper getdHelper() {
		return mDbHelper;
	}
...
//		loadHistoryMessage();
	 ...
	 *            message to log
	 */
	private void logMessage(fm.jihua.chat.service.Message message) {
	 ...
	 */
	private fm.jihua.chat.service.Message otrEncryptMessage(
			fm.jihua.chat.service.Message unencrypted) {

		// if (mOtrSessionId != null && unencrypted != null &&
		// unencrypted.getBody() != null) {
		// try {
		// String body =
		// BeemOtrManager.getInstance().getOtrManager().transformSending(mOtrSessionId,
		// unencrypted.getBody());
...
//			String body;
	...
	// if (mOtrSessionId == null) {
	// mOtrSessionId = new SessionID(mAccountUser, mParticipant.getJIDWithRes(),
	// PROTOCOL);
	...
	// return null;
	// return
	// BeemOtrManager.getInstance().getOtrManager().getSessionStatus(mOtrSessionId).toString();
	// }
}
