...
public final class CameraUtil {
	...
	public static String resolvePhotoFromIntent(final Context ctx, final Intent data, final String path) {
		if (ctx == null || data == null || path == null) {
		...
		if (cu != null && cu.getCount() > 0) {
			try {
				...
				Log.d(TAG, "orition: " + cu.getString(cu.getColumnIndex(MediaStore.Images.ImageColumns.ORIENTATION)));
			...
			if (!(new File(filePath)).exists()) {
				...
				FileUtils.moveFile(new File(filePath), new File(path));
			}
